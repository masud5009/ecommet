...
use App\Http\Controllers\User\Ai\AiContentController;
use App\Http\Controllers\User\Ai\AiImageController;

Route::group(['prefix' => 'user', 'middleware' => ['auth', 'userstatus', 'Demo', 'userLanguage']], function () {

    Route::post('/ai/generate/image', [AiImageController::class, 'generateImage'])
        ->name('user.ai.generate.category.image')->middleware('aiQuotaWarning');
    Route::post('/ai/generate/content', [AiContentController::class, 'generateContent'])
        ->name('user.ai.generate.content')
        ->middleware('aiQuotaWarning');
    Route::post('/user/ai/generate-slider-images', [AiImageController::class, 'generateSliderImages'])
        ->name('user.ai.generate.slider.images')->middleware('aiQuotaWarning');
    // user theme change
