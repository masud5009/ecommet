                          ...
                          <div class="list-item-area mb-20">
                            <ul class="item-list list-unstyled p-0 toggle-list" data-toggle-list="amenitiesToggle"
                              ...
                              @php
                                $aiFeatureEnabled =
                                    is_array($pFeatures) &&
                                    in_array('AI Content & Image Generator', $pFeatures);
                                $aiTokenLimitLabel =
                                    $package->ai_token_limit != '999999'
                                        ? $package->ai_token_limit
                                        : __('Unlimited');
                                $aiImageLimitLabel =
                                    $package->ai_image_limit != '999999'
                                        ? $package->ai_image_limit
                                        : __('Unlimited');
                              @endphp
                              @if (is_array($allPfeatures) && in_array('AI Content & Image Generator', $allPfeatures))
                                @if ($aiFeatureEnabled)
                                  <li class="ai-parent"><i
                                      class="fal fa-check"></i>{{ __('AI Content & Image Generator') }}</li>
                                  <li class="ai-child"><i class="fal fa-check"></i>{{ __('AI Engine') . ' :' }}
                                    {{ $package->ai_engine ? strtoupper($package->ai_engine) : __('N/A') }}</li>
                                  <li class="ai-child"><i class="fal fa-check"></i>{{ __('AI Token Limit') . ' :' }}
                                    {{ $aiTokenLimitLabel }}</li>
                                  <li class="ai-child"><i class="fal fa-check"></i>{{ __('AI Image Limit') . ' :' }}
                                    {{ $aiImageLimitLabel }}</li>
                                @else
                                  <li class="disabled"><i class="fal fa-times"></i>{{ __('AI Content & Image Generator') }}
                                  </li>
                                @endif
                              @endif

                              @php
                                $hideFeatures = ['Posts Limit', 'Blog', 'Custom Page', 'AI Content & Image Generator'];
                              @endphp
