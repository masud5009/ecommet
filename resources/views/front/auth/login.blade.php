        ...
        <form id="#authForm" action="{{ route('user.login') }}" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group mb-30">
            <input type="email" name="email" class="form-control" placeholder="{{ __('Email Address') }}"
              value="{{ env('DEMO_MODE') == 'active' ? 'manti@example.com' : '' }}" required>
            @if (Session::has('err'))
          ...
          <div class="form-group mb-30">
            <input type="password" name="password" class="form-control" placeholder="{{ __('Password') }}" value="{{ env('DEMO_MODE') == 'active' ? '12345678' : '' }}" required>
            @error('password')
              <p class="text-danger mb-2 mt-2">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div class="row align-items-center">
            ...
            <div class="col-8 col-xs-12">
              <div class="link go-signup">
                {{ __("Don't have an account?") }} <a href="{{ route('front.pricing') }}">{{ __('Click Here') }}</a>
                {{ __('to Signup') }}
              </div>
            </div>
          </div>
