...
trait BuildsImagePrompt
{
  ...
  protected function ensureStorageDirectory(string $path = 'ai/categories'): void
  {
    Storage::disk('public')->makeDirectory($path);

    $publicStoragePath = public_path('storage/' . trim($path, '/'));
    if (!is_dir($publicStoragePath)) {
      @mkdir($publicStoragePath, 0775, true);
    }
  }

  protected function mirrorToPublicStorage(string $storagePath): void
  {
    $relative = ltrim($storagePath, '/');
    ...
    $publicReal = realpath($publicStorageRoot);
    $storageReal = realpath($storagePublicRoot);

    if ($publicReal && $storageReal && $publicReal === $storageReal) {
      return;
    }
